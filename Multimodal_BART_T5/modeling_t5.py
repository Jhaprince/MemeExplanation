...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, fusion_layer=None, use_img_trans=None, use_forget_gate=None, cross_attn_type=None, dim_common=768, n_attn_heads=1):
        ...
        if not self.is_decoder:
            if self.use_img_trans:
                self.img_transformer = ImageTransformerEncoder(d_model=352, num_layers=4, num_heads=8, dim_feedforward=352)

            # Some global variables
            visual_feature_dim = 352
            ...
            if cross_attn_type == 0:
                ...
                self._linear_1 = nn.Linear(visual_feature_dim, text_feature_dim)
                self._cbp = CompactBilinearPooling(text_feature_dim,text_feature_dim,text_feature_dim)
    ...
    def forward(
        ...
        hidden_states = self.dropout(hidden_states)
            ...
            #print("textual features shape",hidden_states.shape)
        ...
        if not self.is_decoder:
            ...
            if self.fusion_layer:
                if self.cross_attn_type == 0:
                    ...
                    #print("img",image_features_transformed.shape)
                    ...
                    #print("image",image_features.shape)
                    #print("text",hidden_states.shape)
                    ...
                    attn = torch.bmm(hidden_states, image_features_transformed.transpose(1, 2))
                    attn = F.softmax(attn, dim=1)
                    image_features = torch.bmm(attn, image_features_transformed) # (S_t, D_v)
                    ...
                    #print(conv1.unsqueeze(1).permute(0,3,1,2).shape)
                    #cbp = self._cbp(image_features.unsqueeze(1).permute(0,3,1,2),hidden_states.unsqueeze(1).permute(0,3,1,2)).squeeze()
                    #print(cbp.shape)
                    cbp = image_features*hidden_states
                    signed_cbp = torch.mul(torch.sign(cbp),torch.sqrt(torch.abs(cbp)+1e-12)) 
                    output = signed_cbp
